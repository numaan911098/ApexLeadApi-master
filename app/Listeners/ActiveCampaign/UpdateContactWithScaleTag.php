...
class UpdateContactWithScaleTag
{
    /**
     ...
     * @return void
     */
    public function handle(ScaleUserRegistered $event)
    {
    ...
    private function updateContactTag(User $user)
    {
        ...
        if ($tagId == config('leadgen.active_campaign_scale_paused_tag_id')) {
            $this->removeScalePausedTag($contactId, $user);
        } else {
            $this->removeFreeTag($contactId, $user);
        }

        $this->applyScaleTag($contactId, $user);
    }
    ...
    private function removeFreeTag(int $contactId, User $user)
    {
        ...
        if ($user->isOneToolUser()) {
            $tagIdScaleLiteTrial = config('leadgen.active_campaign_onetool_scale_lite_trial_tag_id');

            $contactTagId = $this->activeCampaign->getContactTagId($tagIdScaleLiteTrial, $contactId);
     ...
     * @return void
     */
    private function applyScaleTag(int $contactId, User $user)
    {
        if ($user->isOneToolUser()) {
            $tagId = config('leadgen.active_campaign_onetool_scale_lite_tag_id');
        } else {
      ...
      /**
     * Remove ScalePaused tag in Active campaign.
     *
     * @param integer $contactId Active campaign contact Id.
     * @return void
     */
    private function removeScalePausedTag(int $contactId, User $user)
    {
        $tagId = intval(config('leadgen.active_campaign__paused_tag_id'));
        $contactTagId = $this->activeCampaign->getContactTagId($tagId, $contactId);
