...
class UpdateContactWithProTrialTag
{
    /**
     ...
     * @return void
     */
    public function handle(ProTrialUserRegistered $event)
    {
        $this->updateContactTag($event->user);
    }

    /**
     * Update Contact to ProTrial and remove Free tag in Active Campaign.
     *
     * @param User $user
     * @return void
     */
    private function updateContactTag(User $user)
    {
        ...
        if ($tagId == config('leadgen.active_campaign_pro_trial_cancelled_tag_id')) {
            $this->removeProTrialCancelledTag($contactId, $user);
        } else {
            $this->removeFreeTag($contactId, $user);
        }

        $this->applyProTrialTag($contactId, $user);
    }
     ...
     * @return void
     */
    private function applyProTrialTag(int $contactId, User $user)
    {
        if ($user->isOneToolUser()) {
      ...
      /**
     * Remove cancelled tag in Active campaign.
     *
     * @param integer $contactId Active campaign contact Id.
     * @return void
     */
    private function removeProTrialCancelledTag(int $contactId, User $user)
    {
        $tagId = intval(config('leadgen.active_campaign_pro_trial_cancelled_tag_id'));
        $contactTagId = $this->activeCampaign->getContactTagId($tagId, $contactId);
